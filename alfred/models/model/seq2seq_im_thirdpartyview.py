...
class Module(seq2seq_im_moca_semantic):
    ...
    def _load_meta_data(self, root, list_img_traj, im, device):
        def sequences_to_one(META_DATA_FILE="all_meta_data.json",
                             ...
                             FEAT_EXPLORATION_CONV="feat_exploration_conv"):
            all_meta_data_path = os.path.join(root, META_DATA_FILE)
            if os.path.isfile(all_meta_data_path):
                ...
                all_meta_data["exploration_imgs"] = frame
        ...
        def agent_sequences_to_one(META_DATA_FILE="meta_agent.json",
            ...
            for i, dict_frame in enumerate(list_img_traj):
                ...
                if os.path.isfile(file_path):
                    with open(file_path, 'r') as f:
                        meta_data = json.load(f)
                    all_meta_data["agent"].append(meta_data)
                else:
                    print("file is not exist: {}".format(file_path))
            all_meta_data["agent"].append(meta_data)
            exploration_path = os.path.join(root, EXPLORATION_META, "*.json")
        ...
        all_meta_data_2 = sequences_to_one(
            ...
            FEAT_EXPLORATION_CONV="feat_exploration_conv_2")
        # agent
        agent_meta_data = agent_sequences_to_one(META_DATA_FILE="meta_agent.json",
                                                 SGG_META="agent_meta",
                                                 EXPLORATION_META="exploration_agent_meta")
        # data
        all_meta["agent_sgg_meta_data"] = agent_meta_data
        all_meta["all_meta_data"] = all_meta_data
        all_meta["all_meta_data_1"] = all_meta_data_1
        all_meta["all_meta_data_2"] = all_meta_data_2
        return all_meta
